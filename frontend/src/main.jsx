...
function Feed({ incidents, selected, setSelected }) {
  return <section className="panel feed"><div className="panel-head"><div><span className="eyebrow">Live stream</span><h2>Incident feed</h2></div><span className="live"><i /> polling every 10s</span></div>
    {!incidents ? <Empty error="Loading incidents…" /> : incidents.length === 0 ? <Empty /> : <div className="table-wrap"><table><thead><tr><th>Workload</th><th>Fault</th><th>Severity</th><th>Detected</th><th>Status</th></tr></thead><tbody>{incidents.map(item => <tr className={selected?.id === item.id ? 'active' : ''} key={item.id} onClick={() => setSelected(item)}><td><strong>{item.pod_name || 'Unknown workload'}</strong><small>{item.namespace || 'cluster'}</small></td><td>{pretty(item.fault_type)}</td><td><Status tone={`severity-${item.severity}`}>{item.severity}</Status></td><td>{time(item.detected_at)}</td><td><Status>{item.remediation_status}</Status></td></tr>)}</tbody></table></div>}
  </section>
}
...
function Detail({ incident }) {
  ...
  return <section className="panel detail"><div className="panel-head"><div><span className="eyebrow">Incident #{incident.id}</span><h2>{pretty(incident.fault_type)}</h2></div><Status tone={`severity-${incident.severity}`}>{incident.severity}</Status></div><div className="facts"><div><label>Root cause</label><strong>{incident.root_cause || 'Unavailable'}</strong></div><div><label>Category</label><strong>{pretty(incident.root_cause_category)}</strong></div><div><label>Confidence</label><strong>{pretty(incident.confidence)}</strong></div><div><label>Remediation</label><strong>{pretty(incident.remediation_status)}</strong></div></div><div className="action"><label>Recommended action</label><code>{incident.recommended_action || 'No action supplied'}</code></div><div className="trace"><label>LLM reasoning trace</label><pre>{JSON.stringify(trace, null, 2)}</pre></div></section>
}
